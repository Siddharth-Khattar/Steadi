...
status: COMPLETE (revised during execution — see 03-04-SUMMARY.md for actual outcome)
files_modified:
  - src/overlay/App.tsx
  - src/overlay/hooks/useOverlayControls.ts
autonomous: false

# REVISION NOTES (2026-02-16):
# - Hover-to-pause replaced with click-to-toggle (hover conflicts with click interactions)
# - Bracket keys replaced with Cmd/Ctrl +/- for font size; plain +/- for opacity
# - Global shortcuts remapped: S→F (overlay), D→S (speed), A→R (rewind)
# - Editor FAB and multi-monitor cycling added (not in original plan)
# - Top-edge fade removed
# - See 03-04-SUMMARY.md for complete deviation log
...
must_haves:
  truths:
    ...
    - "User can rewind in the script with a keyboard shortcut"
    - "Clicking the overlay content area toggles play/pause (click-to-toggle)"
  ...
  artifacts:
    - path: "src/overlay/hooks/useOverlayControls.ts"
      provides: "Local keyboard event handlers and click-to-toggle logic for overlay window"
